...
blink::UserAgentMetadata GetUserAgentMetadata(bool only_low_entropy_ch) {
  ...
  // === GHOST SHELL: UA Client Hints overlay (LOW-ENTROPY) ===
  // Apply BEFORE early-returns below. Otherwise:
  //   • only_low_entropy_ch=true  (called by network::ClientHints to
  //     attach Sec-CH-UA / Sec-CH-UA-Platform / Sec-CH-UA-Mobile to
  //     subresource requests) returns metadata WITHOUT spoof
  //   • custom_ua present (when callers pass --user-agent) also
  //     returns early, again skipping our overlay
  //
  // Low-entropy hints feed navigator.userAgentData.{brands,platform,
  // mobile}. These are the fields the ua_ch_brands_present /
  // ua_ch_platform_matches / ua_ch_mobile_matches selfchecks read.
  // Putting the overlay only at the BOTTOM of the function (before
  // the original early-returns) caused all three checks to fail
  // because the early-returns fire first and the spoof never lands.
  {
    auto& gs_ua = embedder_support::GhostShellUAOverride::GetInstance();
    if (gs_ua.IsActive()) {
      if (!gs_ua.GetBrandList().empty()) {
        metadata.brand_version_list = gs_ua.GetBrandList();
      }
      if (!gs_ua.GetPlatform().empty()) {
        metadata.platform = gs_ua.GetPlatform();
      }
      metadata.mobile = gs_ua.GetMobile();
    }
  }
  ...
  //
  // GHOST SHELL: when our overlay is active we SUPPRESS the
  // kUACHOverrideBlank reset. Otherwise a stealth profile that
  // happens to also pass --user-agent (we do, for the legacy UA
  // string) would zero out the brands/platform/mobile we just set.
  std::optional<std::string> custom_ua = GetUserAgentFromCommandLine();
  if (custom_ua.has_value()) {
    auto& gs_ua = embedder_support::GhostShellUAOverride::GetInstance();
    if (!gs_ua.IsActive() &&
  ...
  // === GHOST SHELL: UA Client Hints overlay (HIGH-ENTROPY) ===
  // High-entropy hints feed navigator.userAgentData.getHighEntropyValues()
  // — platformVersion, fullVersionList, architecture, bitness, model,
  // wow64. The ua_ch_platform_version_matches and ua_ch_full_version_matches
  // selfchecks read these.
  {
    auto& gs_ua = embedder_support::GhostShellUAOverride::GetInstance();
    if (gs_ua.IsActive()) {
      if (!gs_ua.GetBrandFullVersionList().empty()) {
        metadata.brand_full_version_list = gs_ua.GetBrandFullVersionList();
      }
      if (!gs_ua.GetFullVersion().empty()) {
        metadata.full_version = gs_ua.GetFullVersion();
      }
      if (!gs_ua.GetPlatformVersion().empty()) {
        metadata.platform_version = gs_ua.GetPlatformVersion();
      }
      if (!gs_ua.GetArchitecture().empty()) {
        metadata.architecture = gs_ua.GetArchitecture();
      }
      if (!gs_ua.GetBitness().empty()) {
        metadata.bitness = gs_ua.GetBitness();
      }
      metadata.model = gs_ua.GetModel();
      metadata.wow64 = gs_ua.GetWoW64();
    }
  }
  // === END GHOST SHELL ===
...
std::string GetAndroidOSInfo(
  ...
  if (include_android_model == IncludeAndroidModel::Include) {
    std::string android_device_name = BuildModelIn